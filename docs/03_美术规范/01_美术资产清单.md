...
| 视口分辨率 | **320 × 180**（16:9 像素艺术基准分辨率，M4 用户精调由 480×270 收窄） |
| 窗口分辨率 | 2560 × 1440（8× 整数缩放） |
...
> **关键设计约束**：视口 `320×180` 决定了所有美术资产的"游戏内显示尺寸"基准——
> 瓦片网格 **16×16 px**（一行 20 格、一列 ~11 格）；
...
**`[P1]` 视差滚动**：尺寸 480×272 大于当前视口 320×180（M4 用户精调后收窄），可作为最远层视差背景（`ParallaxLayer` 的 `motion_scale` 设为 0.1~0.3），背景图比视口宽能完整覆盖且有余量。如需循环无缝，需检查左右边缘是否能拼接（当前为单张非循环图，横向滚动时可能出现接缝，后续可考虑扩展为 2× 宽）。
